...
function f.dazu(tank, name, label, menge, maxmenge, einheit)
    for i, k in pairs(tank) do
        if name == k.name then
            tank[i].menge    = tank[i].menge    + menge
            tank[i].maxmenge = tank[i].maxmenge + maxmenge
            return
        end
    end
                ...
                if type(k.getFluidInTank(side)) == "table" then
                    for a, b in pairs(k.getFluidInTank(side)) do
                        if type(a) == "number" then
                            f.dazu(tank, b.name, b.label, b.amount, b.capacity, "mb")
                        end
                    end
                end
            end
                ...
                if k[typ[2]] then
                    for _, b in pairs(k[typ[2]]()) do
                        if type(b) == "table" then
                            ...
                            f.dazu(tank, b.name, b.label, b.amount, b.amount, typ[1])
                        end
                    end
                end
            end
    ...
    for _, CompName in pairs({
        ...
        for adresse in pairs(component.list(CompName)) do
            ...
            if name then
                ...
                local maxmenge = 64
                f.dazu(tank, name, label, menge, maxmenge, "")
            end
        end
    end
    ---------------------------------------------------------------------------
